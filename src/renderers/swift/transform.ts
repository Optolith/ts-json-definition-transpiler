...
import { isCodeBlockNode, isOptionalTypeNode } from "./ast/guards.js"
import {
  AttributeListNode,
...
import { joinTypes } from "./ast/utils.js"
import { SwiftOptions } from "./main.js"

const IGNORE_ENV = "swift"

const accessControlModifier = (
  options: SwiftOptions
): DeclModifierNode[] | undefined =>
  options.defaultPublic === true
    ? [declModifier(keywordToken(Keyword.public))]
    : undefined
  ...
  switch (node.kind) {
    ...
    case SourceAst.NodeKind.Dictionary:
      return dictionaryType(
        ...
        childNodeToTypeNode(node.children, options)
      )
      ...
      return identifierType(
        ...
        node.typeArguments === undefined
          ...
          : node.typeArguments.map((arg) =>
              genericArgument(childNodeToTypeNode(arg, options))
            )
      )
    ...
    case SourceAst.NodeKind.Tuple:
      return tupleType(
        node.children.map((child) =>
          tupleTypeElement(childNodeToTypeNode(child, options))
        )
      )
    case SourceAst.NodeKind.Intersection:
      return compositionType(
        node.children.map((child) =>
          compositionTypeElement(childNodeToTypeNode(child, options))
        )
      )
  }
}

const childNodeToExprNode = (
  node: SourceAst.ChildNode,
  options: SwiftOptions
): ExprNode => {
  switch (node.kind) {
        ...
        node.typeArguments === undefined
          ...
          : node.typeArguments
              ...
              .map((arg) =>
                declNameArgument(
                  identifierToken(
                    forcePascal(renderQualifiedName(arg.name), options)
                  )
                )
              )
      )
    ...
    case SourceAst.NodeKind.Tuple:
      return tupleExpr(
        node.children.map((child) =>
          labeledExpr(childNodeToExprNode(child, options))
        )
      )
    case SourceAst.NodeKind.Intersection:
      throw new Error(
        "Intersections are not supported in expression position in Swift"
      )
  }
}
  ...
  switch (node.kind) {
        ...
        filterNonNullable([
          ...variables.map((variable) =>
            variableDecl(
              ...
              [
                patternBinding(
                  identifierPattern(variable.identifier),
                  variable.type
                ),
              ]
            )
                ...
                functionSignature(
                  variables.map((variable) =>
                    functionParameter(
                      ...
                      undefined,
                      {
                        defaultValue: isOptionalTypeNode(variable.type)
                          ? nilLiteralExpr
                          : undefined,
                      }
                    )
    ...
    case SourceAst.NodeKind.Union:
      if (
        ...
        const enumCases = (node.children as SourceAst.RecordNode[]).map(
          (member) => {
            const tag = (
              member.members.find(
                (member) =>
                ...
                [
                  enumCaseElement(
                    ...
                    enumCase.value.kind === SourceAst.NodeKind.Record &&
                      ...
                      : [
                          enumCaseParameter(
                            childNodeToTypeNode(enumCase.value, options)
                          ),
                        ]
                  ),
                ]
                          ...
                          switchCase(
                            ...
                            [
                              infixOperatorExpr(
                                ...
                                assignmentExpr,
                                (enumCase.value.kind ===
                                  SourceAst.NodeKind.Record &&
                                  enumCase.value.members.length > 0) ||
                                  enumCase.value.kind ===
                                    SourceAst.NodeKind.Reference
                                  ? functionCallExpr(
                                      memberAccessExpr(
                                        declReferenceExpr(
                                          identifierToken(enumCase.identifier)
                                        ...
                                        labeledExpr(
                                          tryExpr(
                                            functionCallExpr(
                                              memberAccessExpr(
                                                declReferenceExpr(
                                                  identifierToken("decode")
                                                ),
                                                declReferenceExpr(
                                                  identifierToken("container")
                                                )
                                              ),
                                              [
                                                labeledExpr(
                      ...
                      patternBinding(
                        ...
                        tryExpr(
                          functionCallExpr(
                            memberAccessExpr(
                              declReferenceExpr(
                                identifierToken("singleValueContainer")
                              ),
                              declReferenceExpr(identifierToken("decoder"))
                            )
                          )
                    ...
                    expressionStmt(
                      node.children.reduceRight(
                        (acc: IfExprNode | CodeBlockNode, ref): IfExprNode => {
                          const identifier = forceCamel(
                            renderQualifiedName(ref.name),
                            options
                          )
                        ...
                        codeBlock([
                          throwStmt(
                            functionCallExpr(
                              memberAccessExpr(
                                declReferenceExpr(
                                  identifierToken("dataCorruptedError")
                                ),
                                declReferenceExpr(
                                  identifierToken("DecodingError")
                                )
                              ),
                              [
                                labeledExpr(
